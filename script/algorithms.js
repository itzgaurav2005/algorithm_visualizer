...
class Algorithms {
    ...
    static binaryInsertion(e, order) {
        let elements = e;
        let solution = new Animation();
        ...
        for (let i = 1; i < elements.length; ++i) {
            ...
            let pos = binarySearch(elements, key, 0, j);
            ...
            elements[j + 1] = key;
            solution.addFrame(new Frame([j + 1], [j + 1]));
        }
    
        return solution;
    }
    
    static binarySearch(e, order) {
        let elements = e;
        let solution = new Animation();
        let target = elements[Math.floor(Math.random() * elements.length)]; // Pick random element to search
        let left = 0;
        ...
        while (left <= right) {
            ...
            // Highlight left, mid, right pointers
            solution.addFrame(new Frame([], [left, mid, right]));
    
            if (elements[mid] === target) {
                solution.addFrame(new Frame([mid], [mid])); // Found
                break;
            }
    
            if ((order === "asc" && elements[mid] < target) || (order === "desc" && elements[mid] > target)) {
                left = mid + 1;
            } else {
                right = mid - 1;
            }
